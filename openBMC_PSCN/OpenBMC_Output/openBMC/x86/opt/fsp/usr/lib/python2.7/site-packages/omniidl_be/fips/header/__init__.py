...
# output generation
import omniidl_be.fips.header.opers
import omniidl_be.fips.header.poa
import omniidl_be.fips.header.obv
import omniidl_be.fips.header.tie
import omniidl_be.fips.header.forward
import omniidl_be.fips.header.marshal
import omniidl_be.fips.header.tcstring
import omniidl_be.fips.header.defs

from omniidl_be.fips.header import template

from omniidl_be.fips import config, output, ast, id
...
def defs_fragment(stream, tree):
    ...
    import omniidl_be
    forward = omniidl_be.fips.header.forward.__init__(stream)
    tree.accept(forward)
    
    # generate the bounded string tcParser stuff
    tcstring = omniidl_be.fips.header.tcstring.__init__(stream)
    tree.accept(tcstring)

    defs = omniidl_be.fips.header.defs.__init__(stream)
    tree.accept(defs)
...
def opers_fragment(stream, tree):
    ...
    opers = omniidl_be.fips.header.opers.__init__(stream)
    tree.accept(opers)

    marshal = omniidl_be.fips.header.marshal.__init__(stream)
    tree.accept(marshal)
...
def poa_fragment(stream, tree):
    ...
    poa = omniidl_be.fips.header.poa.__init__(stream)
    tree.accept(poa)
...
def monolithic(stream, tree):
    ...
    def main_obv(stream = stream, tree = tree):
        obv = omniidl_be.fips.header.obv.__init__(stream)
        tree.accept(obv)

    def other_tie(stream = stream, tree = tree):
        if config.state['Normal Tie'] and config.state['BOA Skeletons']:
            tie = omniidl_be.fips.header.tie.BOATieTemplates(stream)
            tree.accept(tie)
        
        if config.state['Flattened Tie']:
            tie = omniidl_be.fips.header.tie.FlatTieTemplates(stream)
            tree.accept(tie)

...
from omniidl_be.fips import cxx, ast, output, id, config, skutil, types
from omniidl_be.fips.skel import template

# XXX Cannot just use from omniidl_be.fips import iface as it doesn't
#     seem to work if a relative path is given to the -p option of omniidl
import omniidl_be.fips.iface
...
def visitInterface(node):
    ...
    I = omniidl_be.fips.iface.Interface(node)
    I_Helper = omniidl_be.fips.iface.instance("I_Helper")(I)
    I_Helper.cc(stream)
    ...
    if node.local():
        _nil_I = omniidl_be.fips.iface.instance("_nil_I")(I)
        _nil_I.cc(stream)

    else:
        _objref_I = omniidl_be.fips.iface.instance("_objref_I")(I)
        _objref_I.cc(stream)

        _pof_I = omniidl_be.fips.iface.instance("_pof_I")(I)
        _pof_I.cc(stream)

        _impl_I = omniidl_be.fips.iface.instance("_impl_I")(I)
        _impl_I.cc(stream)
...
def visitValue(node):
    from omniidl_be.fips import value
